# Troubleshooting — voice2cc

When something doesn't work, the file to read first is `voice2cc.log` in the install dir. Tray → **Open log**.
...
| Antivirus blocks pynput keyboard hook | Whitelist `voice2cc.exe` (or `python.exe` if running from source) |
| Tray icon missing | Open `voice2cc.log` — if `pystray unavailable`, `pip install pystray` |
| Two voice2cc instances running | Task Manager → kill one. Hotkey is global; first listener wins |
...
- `voice2cc.log` lines for the failing paste (look for `focus restore hwnd=...`)
...
```
[INFO] voice2cc.main: continuous mode ON (vad threshold=0.0150)
[INFO] voice2cc.vad: VAD: speech_start (rms=0.04 zcr=0.10)   ← what you should see
```

If you see `continuous mode ON` but no `speech_start` for minutes:
- Your mic level is too low → speak louder, or lower `VOICE2CC_VAD_THRESHOLD` to e.g. 0.008
- Wrong mic selected → Tray → Settings → Mic device

If you see `speech_start` but never `speech_end` (recording forever):
- Background noise stays above threshold → raise `VOICE2CC_VAD_THRESHOLD` to 0.025–0.030
- Or use a quieter mic / closer to mouth

## "VAD triggers on every breath"

- Raise `VOICE2CC_VAD_MAX_ZCR` to 0.13 (stricter ZCR gate)
- Or raise `VOICE2CC_VAD_MIN_SPEECH_MS` to 500 (short breaths under 500 ms ignored)
...
Include:
1. voice2cc version (`v0.4.1` or commit hash)
2. Windows version (`winver`)
3. Last ~50 lines of `voice2cc.log` (mask any API key)
4. What you expected vs. what happened
5. Screenshot of widget state if it looks wrong

Issues: https://github.com/lfzds4399-cpu/voice2cc/issues
